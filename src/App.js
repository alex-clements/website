...
function App() {
  ...
  const createMenuItems = () => {
    ...
    for (var i = 0; i < windows.length; i++) {
      iconList.push({
        activeStatus: windows[i]["activeStatus"],
        activeID: windows[i]["activeID"],
        icon: windows[i]["icon"],
    ...
    setWindows((windows) =>
      windows.map((el) =>
        el["activeID"] == activeID
          ? { ...el, menuIconX: leftPosition, menuIconY: topPosition }
          : { ...el }
      )
    );
  };
      ...
      for (var i = 0; i < newFileStructure.children.length; i++) {
        if (newFileStructure.children[i].name == targetId) {
          for (var j = 0; j < newFileStructure.children[i].data.length; j++) {
            if (
              newFileStructure.children[i].data[j]["fileId"] == parseInt(fileId)
            ) {
              return;
            }
          }
  ...
  return (
    <div className="App">
      ...
      <MenuBar
        onShutDown={handleShutdownScreen}
        menuItems={createMenuItems()}
        onIconSelect={changeActiveItemId}
